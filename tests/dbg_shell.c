...
static void mount() {
  ...
  if (ret == 1) fprintf(stderr, "%s is already mounted.\n", fstype);
}
...
static bool read_command(char* buf, size_t size) {
  ...
  if (cmd_len == 0) {
    return false;
  } else if (cmd_len < MAX_BUF) {
    return true;
  }

  fprintf(stderr, "Maximum input length exceeded (> %d)\n", MAX_BUF - 1);
  while (true) {
    int c = getchar();
    if (c == '\n' || c == EOF) {
      break;
...
//   while (true) {
//     printf("%s ", PROMOTE);
//     while (!read_command(input, MAX_BUF))
//       ;
//
...
int main() {
  char* commands[] = {
    ...
    "cat /proc/iomem",
    "ls /sysfs/devices",
    "ls /sysfs/devices/pci0000:00"
  };

  for (size_t i = 0; i < sizeof(commands) / sizeof(*commands); i++) {
    printf("%s %s\n", PROMOTE, commands[i]);
    parse_cmd(commands[i]);
    run_cmd();
  }
